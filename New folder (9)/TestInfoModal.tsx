...
interface TestInfoModalProps {
  ...
  /** Section labels present in the test — used to show section-specific scoring */
...
const TestInfoModal: React.FC<TestInfoModalProps> = ({
  ...
  const getTypeLabel = (type: string): string => {
    ...
    switch (label) {
      ...
      case "singleDigitInteger" : return "Single Digit Integer Questions"
  ...
  // Top-level scoring entries (exclude caseStudy_* and section:: keys)
  ...
  // Section-specific scoring entries — grouped by section name
  const sectionScoringEntries = Object.entries(scoring).filter(([type]) => type.includes("::"));
  // Group by section: { [sectionName]: { [qType]: ScoringRule } }
  const sectionScoringBySection: Record<string, Record<string, any>> = {};
  sectionScoringEntries.forEach(([key, rule]) => {
    const sepIdx = key.indexOf("::");
    const sectionName = key.slice(0, sepIdx);
    const qType = key.slice(sepIdx + 2);
    if (!sectionScoringBySection[sectionName]) sectionScoringBySection[sectionName] = {};
    sectionScoringBySection[sectionName][qType] = rule;
  });
  const hasSectionScoring = sectionScoringEntries.length > 0 &&
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-lg max-h-[80vh] overflow-y-auto bg-white dark:bg-neutral-900 p-0 thin-scroll">
        ...
        <div className="p-6 space-y-6">
                ...
                return (
                  <div key={type}>
                    ...
                    {isCS && caseStudySubEntries.length > 0 && (
                      <div className="mt-2 space-y-2 bg-white dark:bg-zinc-800 p-4 rounded-xl border border-zinc-200 dark:border-zinc-700">
                        ...
                        {caseStudySubEntries.map(([subKey, subRules]) => (
                          <div
                            key={subKey}
                            className="p-3 rounded-xl border border-indigo-100 dark:border-indigo-800/40"
                          >
                            ...
                            <div className="grid grid-cols-3 gap-2 text-xs">
                              ...
                              <div className="flex items-center gap-1 p-1.5 rounded-lg bg-amber-50 dark:bg-amber-900/20">
                                ...
                                <span className="text-amber-700 font-bold">
                                  {subRules.unattempted >= 0
                                    ? `+${subRules.unattempted}`
                                    : subRules.unattempted}
                                </span>
                              </div>
                            </div>
          ...
          {/* Section-Based Scoring — shown when section overrides exist */}
          {hasSectionScoring && (
            <div>
              <div className="flex items-center gap-2 mb-3">
                <div className="h-4 w-1 bg-indigo-500 rounded-full" />
                <h3 className="text-sm font-bold text-zinc-900 dark:text-zinc-100 uppercase tracking-wider">
                  Section Marking Overrides
                </h3>
              </div>
              <div className="space-y-4">
                {Object.entries(sectionScoringBySection).map(([sectionName, qtypes]) => (
                  <div key={sectionName} className="border border-indigo-100 dark:border-indigo-800/40 rounded-xl overflow-hidden">
                    <div className="bg-indigo-50 dark:bg-indigo-900/20 px-4 py-2 flex items-center gap-2">
                      ...
                      <span className="text-xs font-black text-indigo-700 dark:text-indigo-300 uppercase tracking-widest">{sectionName.replace(/_/g, " ")}</span>
                    </div>
                    <div className="p-3 space-y-2">
                      {Object.entries(qtypes).map(([qType, rules]: [string, any]) => (
                        <div key={qType} className="bg-white dark:bg-zinc-800 p-3 rounded-lg border border-zinc-100 dark:border-zinc-700">
                          <div className="flex items-center justify-between mb-2">
                            <span className="text-[10px] font-black uppercase px-2 py-0.5 rounded-full bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600">{qType}</span>
                            <span className="text-xs font-medium text-zinc-500">{getTypeLabel(qType)}</span>
                          </div>
                          <div className="grid grid-cols-3 gap-2 text-xs">
                            <div className="flex items-center gap-1 p-1.5 rounded-lg bg-emerald-50 dark:bg-emerald-900/20">
                              <CheckCircle className="size-3 text-emerald-600" />
                              <span className="text-emerald-700 font-bold">{rules.correct >= 0 ? `+${rules.correct}` : rules.correct}</span>
                            </div>
                            <div className="flex items-center gap-1 p-1.5 rounded-lg bg-red-50 dark:bg-red-900/20">
                              <XCircle className="size-3 text-red-600" />
                              <span className="text-red-700 font-bold">{rules.incorrect >= 0 ? `+${rules.incorrect}` : rules.incorrect}</span>
                            </div>
                            <div className="flex items-center gap-1 p-1.5 rounded-lg bg-amber-50 dark:bg-amber-900/20">
                              <MinusCircle className="size-3 text-amber-600" />
                              <span className="text-amber-700 font-bold">{rules.unattempted >= 0 ? `+${rules.unattempted}` : rules.unattempted}</span>
                            </div>
                          </div>
                        </div>
                      ))}
