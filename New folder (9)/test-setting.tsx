...
export default function TestSettingsPage() {
  ...
  const [selectedBoardFullTest, setSelectedBoardFullTest] = useState<string>("default");
  const [availableBoards, setAvailableBoards] = useState<string[]>(["default"]);
  const [currentBoardSubjects, setCurrentBoardSubjects] = useState<string[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchBoardSubjects = async () => {
      try {
        let url = `/api/syllabus/subjects?class=${encodeURIComponent(selectedGrade)}`;
        if (selectedBoardFullTest !== "default") {
          url += `&examType=${encodeURIComponent(selectedBoardFullTest)}`;
        }
        const res = await fetch(url);
        if (res.ok) {
          const data = await res.json();
          setCurrentBoardSubjects(data.subjects || []);
        }
      } catch (err) {
        console.error("Failed to fetch board subjects", err);
      }
    };
    fetchBoardSubjects();
  }, [selectedGrade, selectedBoardFullTest]);

  const fetchData = async () => {
    try {
      setLoading(true);
      const [configRes, gradesRes, examTypesRes] = await Promise.all([
      ...
      const fetchedBoards = examTypesRes.data?.examTypes || [];
      setAvailableBoards(["default", ...fetchedBoards]);
    } catch {
  ...
  const updateQuestionCount = (section: string, field: string, value: number) => {
    ...
    if (selectedBoard !== "default") {
      // Write into board-specific override
      const existing = currentConfig?.boardQuestionCount?.[selectedBoard]?.[section]
        || currentConfig?.questionCount?.[section]
        || { min: 10, max: 20, default: 15 };
      updateGradeConfig({
        boardQuestionCount: {
          ...(currentConfig?.boardQuestionCount || {}),
          [selectedBoard]: {
            ...(currentConfig?.boardQuestionCount?.[selectedBoard] || {}),
            [section]: { ...existing, [field]: value },
          },
        },
      });
    } else {
  ...
  const updateBoardSubjectQuestionCount = (subjectKey: string, value: number) => {
    const currentConfig = getCurrentGradeConfig();
    const existingBQC = currentConfig?.boardQuestionCount?.[selectedBoardFullTest] || {};
    const existingSQs = (existingBQC.subjectQuestions as Record<string, number> | undefined) || {};
    updateGradeConfig({
      boardQuestionCount: {
        ...(currentConfig?.boardQuestionCount || {}),
        [selectedBoardFullTest]: {
          ...existingBQC,
          subjectQuestions: {
            ...existingSQs,
            [subjectKey]: value,
  ...
  // ── Available syllabus docs for per-section linking ──
  const [syllabusDocsBySubject, setSyllabusDocsBySubject] = useState<Record<string, any[]>>({});
  ...
  // ── Fetch available syllabus docs per subject for per-section dropdowns ──
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      await Promise.all(
        currentConfig.subjects.map(async (subj: string) => {
          const subjKey = subj.toLowerCase().replace(/\s+/g, "_");
          try {
            const res = await fetch(`/api/private/syllabus/lists?class=${encodeURIComponent(selectedGrade)}&subject=${encodeURIComponent(subjKey)}`);
            if (res.ok) {
          ...
          {selectedGrade && currentConfig ? (
            <>
              ...
              <div className="bg-white p-8 rounded-[32px] shadow-sm border border-slate-100 space-y-6">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500">Min / max per section for subject &amp; chapter tests</p>
                    </div>
                  </div>

                  {/* Board pills — shown for all school grades */}
                ...
                {/* Board-override badge */}
                {selectedBoard !== "default" && (
                  <div className="flex items-center gap-2 px-4 py-2 bg-amber-50 border border-amber-200 rounded-xl">
                    <span className="text-xs font-bold text-amber-700">
                      ✏️ Editing overrides for <span className="text-amber-900">{selectedBoard}</span>.
                      Values left blank will fall back to Default counts.
                    </span>
                  </div>
                )}
                ...
                <div className="space-y-4">
                  {currentConfig.sections?.map((section: string) => {
                    const defaultQC = currentConfig.questionCount?.[section] || { min: 10, max: 20, default: 15 };
                    const boardQC = selectedBoard !== "default"
                      ? currentConfig.boardQuestionCount?.[selectedBoard]?.[section]
                      : undefined;
                    // Display: board override if present, else default
                    const qc = boardQC ?? defaultQC;
                    const isBoardOverridden = !!boardQC;
                    return (
                      <div key={section} className={`p-4 rounded-xl border ${
                        isBoardOverridden && selectedBoard !== "default"
              ...
              <div className="bg-white p-8 rounded-[32px] shadow-sm border border-slate-100 space-y-6">
                ...
                <div>
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-3">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {availableBoards.map((b) => (
                        <button
                          key={b}
                          onClick={() => setSelectedBoardFullTest(b)}
                          className={`px-3 py-1 rounded-lg text-xs font-bold border transition-all ${
                            selectedBoardFullTest === b
                              ? "bg-indigo-600 text-white border-indigo-600 shadow"
                              : "bg-white text-slate-500 border-slate-200 hover:border-indigo-300 hover:text-indigo-600"
                          }`}
                        >
                  ...
                  {/* Board-override badge */}
                  {selectedBoardFullTest !== "default" && (
                    <div className="flex items-center gap-2 px-4 py-2 bg-amber-50 border border-amber-200 rounded-xl mb-3">
                      <span className="text-xs font-bold text-amber-700">
                        Editing overrides for <span className="text-amber-900">{selectedBoardFullTest}</span>.
                        Values left blank will fall back to Default.
                      </span>
                    </div>
                  )}

                  <div className="space-y-2">
                    {(!currentBoardSubjects || currentBoardSubjects.length === 0) ? (
                      <p className="text-sm text-slate-400 bg-slate-50 p-4 rounded-xl">
                        No subjects configured for this board/grade.
                      </p>
                    ) : (
                      currentBoardSubjects.map((subj: string) => {
                        const subjectKey = subj.toLowerCase().replace(/\s+/g, "_");
                        const isBoardOverride = selectedBoardFullTest !== "default";
                        const boardSQ = isBoardOverride
                          ? (currentConfig.boardQuestionCount?.[selectedBoardFullTest] as any)?.subjectQuestions?.[subjectKey]
                          : undefined;
                        const count = boardSQ ?? currentConfig.fullTest?.subjectQuestions?.[subjectKey]
                          ?? currentConfig.fullTest?.perSubjectQuestions
                          ?? 0;
                        return (
                          <div key={subjectKey} className={`flex items-center gap-4 px-4 py-3 rounded-xl border ${
                            isBoardOverride && boardSQ != null
                              ? "bg-indigo-50 border-indigo-200"
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <SmartNumberInput
                                ...
                                onChange={(v) =>
                                  isBoardOverride
                                    ? updateBoardSubjectQuestionCount(subjectKey, v)
                                    : updateSubjectQuestionCount(subjectKey, v)
                                }
                    ...
                    <p className="text-[10px] text-slate-400 mt-1">
                      Total: <strong>{
                        selectedBoardFullTest !== "default"
                          ? (() => {
                              const bqc = currentConfig.boardQuestionCount?.[selectedBoardFullTest] as any;
                              return currentBoardSubjects?.reduce((sum: number, subj: string) => {
                                const k = subj.toLowerCase().replace(/\s+/g, "_");
                                const bv = bqc?.subjectQuestions?.[k];
                                return sum + (bv ?? currentConfig.fullTest?.subjectQuestions?.[k] ?? currentConfig.fullTest?.perSubjectQuestions ?? 0);
                              }, 0) || 0;
                            })()
                          : currentBoardSubjects?.reduce((sum: number, subj: string) => {
                              const k = subj.toLowerCase().replace(/\s+/g, "_");
                              return sum + (currentConfig.fullTest?.subjectQuestions?.[k] ?? currentConfig.fullTest?.perSubjectQuestions ?? 0);
                            }, 0) || 0
                      }</strong> questions across {currentBoardSubjects?.length || 0} subject(s)
                    </p>
                  </div>
                </div>
                          ...
                          {sections.length === 0 ? (
                            ...
                            <div className="divide-y divide-slate-50">
                              {sections.map((sec: any, idx: number) => (
                                <div key={idx} className="px-5 py-3 border-b border-slate-50 last:border-b-0">
                                  <div className="flex items-center gap-3">
                                  ...
                                  {/* ── Per-section syllabus linking dropdown ── */}
                                  <div className="flex items-center gap-2 mt-2 ml-10">
                                    <label className="text-[10px] text-slate-400 uppercase tracking-wider font-bold shrink-0">
                                      Syllabus
                                    </label>
                                    <select
                                      value={sec.linkedSyllabusId || ""}
                                      onChange={(e) => updateSubjectSection(subjectKey, idx, "linkedSyllabusId", e.target.value || undefined)}
                                      className="flex-1 p-1.5 bg-white border border-slate-200 rounded-lg text-xs font-medium text-slate-600 focus:ring-2 focus:ring-indigo-500 outline-none"
                                    >
                                      <option value="">AI decides (no link)</option>
                                      {(syllabusDocsBySubject[subjectKey] || []).map((doc: any) => (
                                        <option key={doc._id} value={doc._id}>
                                          {doc.label} ({doc.exam_type || "no type"})
                                        </option>
                                      ))}
                                    </select>
                                    {sec.linkedSyllabusId && (
                                      <button
                                        onClick={() => updateSubjectSection(subjectKey, idx, "linkedSyllabusId", undefined)}
                                        className="text-slate-300 hover:text-red-500 transition-colors shrink-0"
                                        title="Clear linked syllabus"
                                      >
