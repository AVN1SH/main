...
import { PARROT_CHARACTER_SVG } from "@/game/characters/parrot";
import { MAP_2_CONFIG } from "./config";
...
const K = MAP_2_CONFIG.textureKeys;
const tx = (key: string) => K[key] || key;

export class MainGameScene_Map2 extends Phaser.Scene {
  ...
  private dom = new DomLayer(
    MAP_2_CONFIG.domAssets,
    MAP_2_CONFIG.backgroundTall,
  );
  ...
  private domRebuild() {
    ...
    for (let i = 0; i < this.decorImages.length; i++)
      updateEl(this.decorImages[i]);
  }
  ...
  create() {
    ...
    this.physics.add.collider(
      ...
      (p: any, h: any) =>
        h.getData("type") === tx("obstacle_thorns") && !this.hasBalloon,
      this,
    );
    ...
    this.clouds.getChildren().forEach((c: any) => {
      c.body.checkCollision.down =
        c.body.checkCollision.left =
        c.body.checkCollision.right =
          false;
    });
    this.hurdles.getChildren().forEach((h: any) => {
      h.body.checkCollision.down =
        h.body.checkCollision.left =
        h.body.checkCollision.right =
          false;
    });
  ...
  private buildGroundTiles(w: number, h: number) {
    ...
    for (let i = 0; i < numBlocks; i++) {
      ...
      gBlock.body.checkCollision.down =
        gBlock.body.checkCollision.left =
        gBlock.body.checkCollision.right =
          false;
  ...
  private repositionDynamicObjects(newW: number) {
    ...
    this.hurdles
      .getChildren()
      .forEach((h: any) => remapX(h, h.getData("type") === tx("obstacle_bee")));
  ...
  spawnCloud(x: number, y: number, forceSafe: boolean = false) {
    ...
    if (
      ...
      barn.body!.checkCollision.down =
        barn.body!.checkCollision.left =
        barn.body!.checkCollision.right =
          false;
  ...
  hitCloud(
    ...
    if (player.body && player.body.touching.down) {
      ...
      this.tweens.add({
        targets: player,
        scaleX: this.responsiveScale * 0.8,
        scaleY: this.responsiveScale * 1.25,
        duration: 150,
        yoyo: true,
      });
  ...
  hitHurdle(
    ...
    if (type === tx("obstacle_thorns") || type === tx("obstacle_bee")) {
      if (
        type === tx("obstacle_bee") &&
        player.body &&
        player.body.velocity.y < 0
      )
        return;
      if (
        type === tx("obstacle_thorns") &&
        player.body &&
  ...
  collectBalloon(
    ...
    if (this.hasBalloon && this.attachedBalloon) {
      ...
      const bEl = this.attachedBalloon.getData("__dom") as
        | HTMLElement
        | undefined;
      if (bEl) {
        this.tweens.killTweensOf(this.balloonProxy);
        ...
        this.tweens.add({
          ...
          onUpdate: () => {
            bEl.setAttribute(
              "data-anim-scale",
              String(this.balloonProxy.scale),
            );
          },
        });
    ...
    this.attachedBalloon = this.add.image(
      player.x,
      player.y - Math.round(30 * this.responsiveScale),
      tx("balloon"),
    );
  ...
  update() {
    ...
    this.domRebuild();
    if (this.player.y > cam.scrollY + h + 100) {
      this.die();
      return;
    }
    ...
    if (highestCloudY > minY - 300) {
      ...
      if (Math.random() < 0.1) {
        ...
        coin.setScale(this.responsiveScale * 0.15);
