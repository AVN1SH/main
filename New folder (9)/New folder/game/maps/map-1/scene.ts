...
import { SHEEP_CHARACTER_SVG } from "@/game/characters/sheep";
import { MAP_1_CONFIG } from "./config";
...
const K = MAP_1_CONFIG.textureKeys;
const tx = (key: string) => K[key] || key;

export class MainGameScene_Map1 extends Phaser.Scene {
  ...
  // Throttle Storage coin writes (every 3 collects)
  private _pendingCoinDelta = 0;

  private dom = new DomLayer(MAP_1_CONFIG.domAssets, MAP_1_CONFIG.backgroundTall);
  ...
  create() {
    ...
    this.attachDom(this.player, "player", undefined, undefined, true, SHEEP_CHARACTER_SVG);
  ...
  collectBalloon(player: Phaser.Physics.Arcade.Sprite, balloon: Phaser.Physics.Arcade.Sprite) {
    ...
    soundManager.playCollectedBalloon();
    this.balloonBoostY = 0;
    this.attachedBalloon = this.add.image(player.x, player.y - Math.round(50 * this.responsiveScale), tx("balloon"));
  ...
  collectCoin(player: Phaser.Physics.Arcade.Sprite, coin: Phaser.Physics.Arcade.Sprite) {
    ...
    // Batch Storage writes — flush every 3 coins to avoid repeated localStorage I/O
  ...
  enterBarn() {
    ...
    // Commit any un-pushed score & coins before switching scenes
    useGameStore.getState().setScore(this.score);
  ...
  die() {
    ...
    // Flush any pending coin delta before leaving
    if (this._pendingCoinDelta > 0) {
      Storage.setCoins(Storage.getCoins() + this._pendingCoinDelta);
      this._pendingCoinDelta = 0;
    }
    ...
    // Offer the ad-lifeline popup once per round (only in Playables env).
    if (!this.hasUsedAdLifeline) {
      this.hasUsedAdLifeline = true;
      useGameStore.getState().setShowAdLifeline(true);

      // Listen for the player's decision from the UI.
      const onRevive = () => {
        ...
        // Respawn on a safe platform: scroll to the last known safe Y.
  ...
  update() {
    ...
    // Cache frequently-accessed values to avoid property chain lookups
    const w = this.scale.width;
    const h = this.scale.height;
    ...
    if (this.hasBalloon && this.attachedBalloon && this.player.body) {
      ...
      this.attachedBalloon.y = this.player.y - Math.round(50 * this.responsiveScale);
    }

    const px = this.player.x;
    if (px < -24) this.player.x = w + 24;
    else if (px > w + 24) this.player.x = -24;

    if (this.player.y < this.highestY) {
      const diff = this.highestY - this.player.y;
      this.score += Math.floor(diff);
      this.highestY = this.player.y;
      // Throttle score store writes — only push when delta >= 10 pts
    ...
    this.domRebuild(); // per-element dirty check in setPos() skips unchanged positions

    if (this.player.y > cam.scrollY + h + 100) { this.die(); return; }

    // Single-pass culling across all groups
