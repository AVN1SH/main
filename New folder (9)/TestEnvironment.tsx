...
export const getFullTypeLabel = (
  type: string,
  isCaseStudySub?: boolean,
): string => {
  ...
  switch (type) {
    ...
    case "singleDigitInteger":
      return `${prefix}SINGLE DIGIT INTEGER QUESTION`;
    case "multiCorrect":
...
export const calculateQuestionScore = (
  ...
  if (checkCorrectness(q, answer)) {
    ...
    if (q.type === "matchList" && rule.partialPerMatch !== undefined) {
      ...
      maxMark = Math.max(
        maxMark,
        mlQ.correctMatches.length * rule.partialPerMatch,
      );
    }
    if (
      (q.type === "multiCorrect" || q.type === "matchListOptionFormat") &&
      rule.partialPerCorrect !== undefined
    ) {
      const numCorrect =
        q.type === "multiCorrect"
          ? (q as MultiCorrectTestQuestion).correctAnswerIndices.length
          : (
              (q as MatchListOptionFormatTestQuestion).correctIndices ||
              [(q as MatchListOptionFormatTestQuestion).correctIndex].filter(
                (i) => i !== undefined,
              )
            ).length;
      maxMark = Math.max(maxMark, numCorrect * rule.partialPerCorrect);
    }
    if (q.type === "caseStudy" && rule.partialPerCorrect !== undefined) {
      maxMark = rule.partialPerCorrect;
    }
  ...
  switch (q.type) {
    case "multiCorrect": {
      ...
      const hasWrong = selectedIndices.some(
      ...
      if (rule.partialPerCorrect !== undefined) {
        ...
        return {
          marks: correctSelected * rule.partialPerCorrect,
          maxMarks: rule.correct,
        };
      }
      return { marks: rule.incorrect, maxMarks: rule.correct };
    }
    ...
    case "matchListOptionFormat": {
      const mlofQ = q as MatchListOptionFormatTestQuestion;
      const correctIndices =
        mlofQ.correctIndices ||
        (mlofQ.correctIndex !== undefined ? [mlofQ.correctIndex] : []);
      const selected = (answer || "").split(",").filter(Boolean);
      const selectedIndices = selected.map((s) => Number(s));
      const hasWrong = selectedIndices.some(
        (idx) => !correctIndices.includes(idx),
      );
      if (hasWrong) {
    ...
    case "matchList": {
      ...
      if (rule.partialPerMatch !== undefined) {
        ...
        const correctCount = userPairs.filter((p) =>
          correctPairSet.has(p),
        ).length;
        const totalPairs = mlQ.correctMatches.length;
        const maxMarks = Math.max(
          rule.correct,
          totalPairs * rule.partialPerMatch,
        );
    ...
    case "singleDigitInteger":
      return (
        <SingleDigitIntegerQuestion
          question={question as unknown as NumericalTestQuestion}
          currentAnswer={currentAnswer}
          onAnswer={onAnswer}
          isSubmitted={isSubmitted}
          isDisabled={isSubmitted || isViewMode}
...
export const TestEnvironment: React.FC<TestEnvironmentProps> = ({
  ...
  // Derive available sections for the current view (subject or full test)
  const availableSections = React.useMemo(() => {
    ...
    return null;
  }, [isFullSyllabus, questionSections, filteredQuestions]);

  const hasSectionsForCurrentView =
    !!availableSections && availableSections.length > 0;

  // Derive the section of the current question for auto-highlighting
  const activeQuestionSection = hasSectionsForCurrentView
      ...
      const timer = setTimeout(() => {
        ...
        const sortedQuestions = [...normalizedQuestions].sort((a, b) => {
          if (isMultiSubject && subjects && subjects.length > 0) {
            const subjA = (a as any).subject || "";
            const subjB = (b as any).subject || "";
            const idxA = subjA ? subjects.indexOf(subjA) : subjects.length;
            const idxB = subjB ? subjects.indexOf(subjB) : subjects.length;
            const validIdxA = idxA !== -1 ? idxA : subjects.length;
            const validIdxB = idxB !== -1 ? idxB : subjects.length;
            if (validIdxA !== validIdxB) return validIdxA - validIdxB;
          }

          const secA = (a as any).section || "";
          const secB = (b as any).section || "";
          if (questionSections && questionSections.length > 0) {
            const secIdxA = secA
              ? questionSections.indexOf(secA)
              : questionSections.length;
            const secIdxB = secB
              ? questionSections.indexOf(secB)
              : questionSections.length;
            const validSecIdxA =
              secIdxA !== -1 ? secIdxA : questionSections.length;
            const validSecIdxB =
              secIdxB !== -1 ? secIdxB : questionSections.length;
            if (validSecIdxA !== validSecIdxB)
              return validSecIdxA - validSecIdxB;
          } else if (secA !== secB) {
  ...
  const calculateResults = (): TestResults => {
    ...
    questions.forEach((q) => {
      ...
      // Resolution order:
      // 1. section::questionType  (e.g. "Physics A::mcq")
      // 2. caseStudy_subType      (for case study sub-questions)
      // 3. caseStudy              (generic case study fallback)
      // 4. questionType           (e.g. "mcq")
      // 5. hardcoded default
      let rule;
      ...
      if (normSection && scoring[`${normSection}::${q.type}`]) {
        ...
        rule =
          scoring[`caseStudy_${q.type}`] || scoring["caseStudy"] || defaultRule;
      } else {
        rule = scoring[q.type] || defaultRule;
      }
      ...
      if (isLastInSubject && hasNextSubject) {
        ...
        return (
          <button
            ...
            className="flex items-center gap-2 px-4 py-3 rounded-xl bg-indigo-600 text-white font-bold hover:bg-indigo-700 transition-all shadow-lg hover:shadow-xl active:scale-95 whitespace-nowrap"
          >
            Next: {nextSubject}
  ...
  if (isSubmitted) {
    ...
    return (
      <TestReview
        ...
        testData={testData}
        isReview={isReview}
        questionSections={hasSectionsForCurrentView ? availableSections : null}
      />
    );
  }
  ...
  return (
    <>
      <TestInfoModal
        ...
        questionTypes={
          [
            ...
            ...(questions.some((q) => (q as any)._caseStudyParentId)
              ? ["caseStudy"]
              : []),
          ] as TestQuestionType[]
        }
      ...
      <div className="flex flex-col h-full w-full bg-slate-50/30 overflow-hidden relative">
        ...
        <div className="sticky top-16 z-20 flex flex-col lg:flex-row items-center justify-between">
          ...
          {/* Section Navigator (Subject Test only, when questionSections exist, or Full Test dynamically) */}
          {hasSectionsForCurrentView && (
            <div className="sticky top-16 z-20 flex items-center gap-2 px-4 bg-white border-b lg:border-l border-slate-200 w-full">
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        ...
                        const firstIdx = (
                          isMultiSubject ? filteredQuestions : questions
                        ).findIndex((q) => (q as any).section === sec);
                        if (firstIdx !== -1) {
                          trackQuestionTime();
                          if (isMultiSubject) {
                            const actualIndex = questions.indexOf(
                              filteredQuestions[firstIdx],
                            );
              ...
              <div className="flex justify-between items-start mb-6 relative z-10">
                <div className="flex-1">
                  <span className="inline-block text-[10px] font-bold uppercase px-2 py-0.5 rounded-full bg-indigo-100 text-indigo-600 mb-2">
                    {getFullTypeLabel(
                      ...
                      !!currentQuestion._caseStudyParentId,
                    )}
                  </span>
                ...
                <div className="absolute -top-[35px] md:-top-8 -right-1 md:-right-3 bg-white flex items-center gap-2">
                  <div className="flex items-center gap-1.5 font-bold text-xs border px-1 md:px-2 rounded-full py-0.5 md:py-1">
                    ...
                    {(() => {
                      const isCaseStudySub = !!(currentQuestion as any)
                        ._caseStudyParentId;
                      const lookupType = isCaseStudySub
                        ? "caseStudy"
                        : currentQuestion.type;
                      const badgeRule = scoring[lookupType];
                      return (
                        <>
                    ...
                    {(() => {
                      ...
                      const lookupType = isCaseStudySub
                        ? "caseStudy"
                        : currentQuestion.type;
                      const rule = scoring[lookupType];
                      if (!rule) return null;
                      if (
                        (lookupType === "multiCorrect" ||
                          lookupType === "matchListOptionFormat") &&
                        (rule.partialPerCorrect ?? 0) > 0
                      ) {
                        return (
                          <span className="text-amber-600 bg-amber-50 px-2 rounded-lg border border-amber-100">
                            +{rule.partialPerCorrect}/opt
                          </span>
                        );
                      }
                      if (
                        lookupType === "caseStudy" &&
                        (rule.partialPerCorrect ?? 0) > 0
                      ) {
                        return (
                          <span className="text-amber-600 bg-amber-50 px-2 rounded-lg border border-amber-100">
                            +{rule.partialPerCorrect}/sub
                          </span>
                        );
                      }
                      if (
                        lookupType === "matchList" &&
                        (rule.partialPerMatch ?? 0) > 0
                      ) {
                        return (
                          <span className="text-amber-600 bg-amber-50 px-2 rounded-lg border border-amber-100">
                            +{rule.partialPerMatch}/match
                          </span>
                        );
                      }
                      return null;
