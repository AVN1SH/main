...
export default function FeedDatabasePage() {
  ...
  const [inputType, setInputType] = useState<"file" | "text" | "raw" | "json">("file");
  ...
  // Fetch subjects from Syllabus model when grade changes
  useEffect(() => {
    if (!formData.grade) {
      setDbSubjects([]);
      return;
  ...
  const handleProcess = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (inputType === "json") {
      if (!jsonText) {
        toast.error("Please enter JSON data to proceed.");
        return;
      }
      try {
        ...
        if (formData.docType.toLowerCase() === "sqp") {
          if (parsedData && Array.isArray(parsedData.data)) {
            ...
            throw new Error("SQP JSON must be an array of questions or an object with a 'data' array property.");
          }
        } else if (formData.docType.toLowerCase() === "pyq") {
          setPyqResult(parsedData);
        } else {
          toast.error("JSON entry is primarily supported for SQP (and PYQ). Please select a valid Document Type.");
          setAppState(AppState.IDLE);
          return;
        }
      } catch (e: any) {
        const errorMessage = e.message || "Invalid JSON format. Please check your JSON string.";
  ...
  const handleAddSyllabusItem = (e: React.MouseEvent) => {
    ...
    setSyllabusList((prev) => [...prev, { ...syllabusInput, isSubChapters: false, isShowChapterName: false, subChapters: [] }]);
    setSyllabusInput({ chapter: "", weightage: "", description: "", isSubChapters: false, isShowChapterName: false, subChapters: [] });
  };
  ...
  const handleToggleSubChapters = (index: number) => {
    setSyllabusList((prev) =>
      prev.map((item, i) =>
        i === index ? { ...item, isSubChapters: !item.isSubChapters, isShowChapterName: item.isSubChapters ? false : item.isShowChapterName, subChapters: !item.isSubChapters ? item.subChapters || [] : [] } : item,
      ),
    );
                  ...
                  {inputType === "file" ? (
                    ...
                    <div className="space-y-2">
                      <textarea
                        ...
                        placeholder="Enter JSON format for SQP here..."
                        className="w-full h-48 p-4 bg-slate-50 border border-slate-200 rounded-xl text-sm font-mono focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all resize-none"
                        disabled={appState === AppState.PROCESSING}
                      />
                      <p className="text-[10px] text-slate-400 italic">
                        Tip: Valid JSON will bypass Gemini processing and directly render for SQP insertion.
                      </p>
                    </div>
                  )}
                </section>
                      ...
                      {!isCustomYear ? (
                        <div className="flex gap-2">
                          <select
                            ...
                            value={formData.year}
                            onChange={(e) => {
                              if (e.target.value === "__custom_year__") {
                            ...
                            className="flex-1 p-2.5 bg-slate-50 border border-slate-200 rounded-lg text-slate-800 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all invalid:border-red-300 focus:invalid:ring-red-200"
                            disabled={appState === AppState.PROCESSING}
                            required
                          >
                            ...
                            <option value="__custom_year__">✏️ Manual entry</option>
                          </select>
                        </div>
                      ) : (
                        <div className="flex gap-2">
