...
export default function ScoringConfigPage() {
  ...
  // For section scoring — which subject's sections are being edited
  const [selectedSectionSubject, setSelectedSectionSubject] = useState<string>("");
  ...
  // Top-level types (exclude caseStudy_* sub-type keys and section:: override keys)
  const getTopLevelScoring = (grade: string) => {
    const scoring = getScoringForGrade(grade);
    return Object.fromEntries(Object.entries(scoring).filter(([k]) => !k.startsWith("caseStudy_") && !k.includes("::")));
  };
  ...
  // Returns subjects that have enabled sections for the selected grade
  const getSubjectsWithSections = (grade: string): string[] => {
    const subjectSections = testConfig?.grades?.[grade]?.subjectSections || {};
    return Object.keys(subjectSections).filter(
      (subj) => (subjectSections[subj] as any[]).some((s: any) => s.enabled)
    );
  };

  // Returns enabled section names for a subject
  const getEnabledSections = (grade: string, subjectKey: string): string[] => {
    const sections: any[] = testConfig?.grades?.[grade]?.subjectSections?.[subjectKey] || [];
    return sections.filter((s: any) => s.enabled).map((s: any) => s.sectionName as string);
  };

  // Returns section scoring entries for a section: { [qType]: rule }
  const getSectionScoring = (grade: string, sectionName: string) => {
    const scoring = getScoringForGrade(grade);
    const norm = sectionName.toLowerCase().replace(/\s+/g, "_");
    return Object.fromEntries(
      Object.entries(scoring)
        .filter(([k]) => k.startsWith(`${norm}::`))
        .map(([k, v]) => [k.replace(`${norm}::`, ""), v])
    );
  };

  const addSectionQType = (grade: string, sectionName: string, qType: string) => {
    const norm = sectionName.toLowerCase().replace(/\s+/g, "_");
    addQuestionType(grade, `${norm}::${qType}`);
  };

  const removeSectionQType = (grade: string, sectionName: string, qType: string) => {
    const norm = sectionName.toLowerCase().replace(/\s+/g, "_");
    removeQuestionType(grade, `${norm}::${qType}`);
  };

  const availableSectionQTypes = (grade: string, sectionName: string) => {
    const current = getSectionScoring(grade, sectionName);
    return QUESTION_TYPES.filter((qt) => !current[qt]);
  };
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-9 space-y-8">
          ...
          {/* Section-Based Scoring Card — only shown for grades with subjectSections */}
          {(() => {
            const subjectsWithSections = getSubjectsWithSections(selectedGrade);
            if (subjectsWithSections.length === 0) return null;
            // Auto-select first subject if none selected or current is invalid
            const activeSubject = subjectsWithSections.includes(selectedSectionSubject)
              ? selectedSectionSubject
              : subjectsWithSections[0];
            const enabledSections = getEnabledSections(selectedGrade, activeSubject);

            return (
              <div className="bg-white p-8 rounded-[32px] shadow-sm border border-indigo-100">
                {/* Header */}
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-xl font-black text-slate-900">Section Scoring</h3>
                      <p className="text-slate-400 text-xs font-medium">Override marking per section — falls back to default if not set</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex gap-2 flex-wrap mb-6">
                  {subjectsWithSections.map((subj) => (
                    <button
                      key={subj}
                      onClick={() => setSelectedSectionSubject(subj)}
                ...
                <div className="space-y-6">
                  {enabledSections.map((sectionName) => {
                    const sectionScoring = getSectionScoring(selectedGrade, sectionName);
                    const avail = availableSectionQTypes(selectedGrade, sectionName);
                    return (
                      <div key={sectionName} className="border border-indigo-100 rounded-2xl">
                        {/* Section header */}
                        <div className="bg-indigo-50 px-5 py-3 flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <div className="size-2 rounded-full bg-indigo-500" />
                            <span className="text-sm font-black text-indigo-800 uppercase tracking-wider">{sectionName}</span>
                          </div>
                          {avail.length > 0 && (
                            <div className="relative group">
                              <button className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-indigo-200 text-indigo-600 font-bold rounded-lg text-xs hover:bg-indigo-600 hover:text-white transition-all">
                                <Plus className="size-3.5" />Add Rule
                              </button>
                              <div className="absolute right-0 top-full mt-1 w-48 bg-white border border-slate-100 rounded-xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-10">
                                {avail.map((qt) => (
                                  <button
                                    key={qt}
                                    onClick={() => addSectionQType(selectedGrade, sectionName, qt)}
                                    className="w-full text-left px-4 py-2 text-sm font-bold text-slate-600 hover:bg-indigo-50 hover:text-indigo-600 first:rounded-t-xl last:rounded-b-xl"
                                  >{qt}</button>
                                ))}
                              </div>
                        ...
                        <div className="p-4 space-y-3">
                          {Object.entries(sectionScoring).length === 0 && (
                            <p className="text-xs text-slate-400 font-medium text-center py-4">
                              No overrides — using default scoring. Click "Add Rule" to override.
                            </p>
                          )}
                          {Object.entries(sectionScoring).map(([qType, rule]: [string, any]) => (
                            <div key={qType} className="p-4 bg-slate-50 rounded-xl border border-slate-100">
                              <div className="flex items-center justify-between mb-3">
                                <div className="px-3 py-1 bg-indigo-600 text-white rounded-lg text-xs font-black">{qType.toUpperCase()}</div>
                                <button
                                  onClick={() => removeSectionQType(selectedGrade, sectionName, qType)}
                                  className="p-1.5 text-slate-300 hover:text-red-500 transition-all"
                              ...
                              <div className="grid grid-cols-3 gap-3">
                                <div className="space-y-1">
                                  <label className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">Correct (+)</label>
                                  <SmartNumberInput
                                    value={rule.correct ?? 0}
                                    onChange={(v) => updateScoring(selectedGrade, `${sectionName.toLowerCase().replace(/\s+/g, "_")}::${qType}`, "correct", v)}
                                    className="w-full p-2.5 bg-white border border-slate-200 rounded-xl text-center font-black text-emerald-600 focus:ring-2 focus:ring-emerald-500"
                                  />
                                </div>
                                <div className="space-y-1">
                                  <label className="text-[10px] font-black text-red-500 uppercase tracking-widest">Incorrect (-)</label>
                                  <SmartNumberInput
                                    value={rule.incorrect ?? 0}
                                    onChange={(v) => updateScoring(selectedGrade, `${sectionName.toLowerCase().replace(/\s+/g, "_")}::${qType}`, "incorrect", v)}
                                    className="w-full p-2.5 bg-white border border-slate-200 rounded-xl text-center font-black text-red-500 focus:ring-2 focus:ring-red-500"
                                  />
                                </div>
                                <div className="space-y-1">
                                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Unattempted</label>
                                  <SmartNumberInput
                                    value={rule.unattempted ?? 0}
                                    onChange={(v) => updateScoring(selectedGrade, `${sectionName.toLowerCase().replace(/\s+/g, "_")}::${qType}`, "unattempted", v)}
                                    className="w-full p-2.5 bg-white border border-slate-200 rounded-xl text-center font-black text-slate-400 focus:ring-2 focus:ring-slate-500"
                                  />
                                </div>
                              </div>
                              {(qType === "multiCorrect" || qType === "matchListOptionFormat") && (
                                <div className="mt-3 pt-3 border-t border-slate-200 space-y-1">
                                  <label className="text-[10px] font-black text-amber-600 uppercase tracking-widest">Partial per Correct Option</label>
                                  <SmartNumberInput
                                    value={rule.partialPerCorrect ?? 0}
                                    onChange={(v) => updateScoring(selectedGrade, `${sectionName.toLowerCase().replace(/\s+/g, "_")}::${qType}`, "partialPerCorrect", v)}
                                    className="w-full p-2.5 bg-white border border-slate-200 rounded-xl text-center font-black text-amber-600 focus:ring-2 focus:ring-amber-500"
                                  />
                                </div>
                              )}
                              {qType === "matchList" && (
                                <div className="mt-3 pt-3 border-t border-slate-200 space-y-1">
                                  <label className="text-[10px] font-black text-amber-600 uppercase tracking-widest">Partial per Match</label>
                                  <SmartNumberInput
                                    value={rule.partialPerMatch ?? 0}
                                    onChange={(v) => updateScoring(selectedGrade, `${sectionName.toLowerCase().replace(/\s+/g, "_")}::${qType}`, "partialPerMatch", v)}
                                    className="w-full p-2.5 bg-white border border-slate-200 rounded-xl text-center font-black text-amber-600 focus:ring-2 focus:ring-amber-500"
                                  />
                                </div>
                              )}
