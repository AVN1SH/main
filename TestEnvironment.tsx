...
const normalizeQuestion = (q: Record<string, unknown>): TestQuestion => {
  ...
  if (q.type && validTypes.includes(q.type as string)) {
    return { ...q, id: String(q.id) } as TestQuestion;
  }
  // Legacy MCQ format
...
export const checkCorrectness = (
  ...
  switch (q.type) {
    ...
    case "caseStudy": {
      try {
        const subAnswers = JSON.parse(answer);
        return q.subQuestions.every(
          (sub, i) =>
            subAnswers[i] === String.fromCharCode(65 + sub.correctAnswerIndex),
        );
      } catch {
        return false;
      }
...
export const getFullTypeLabel = (type: string): string => {
  switch (type) {
    case "numerical":
      return "NUMERICAL BASED QUESTION";
    case "multiCorrect":
      return "MULTI-CORRECT QUESTION";
    case "matchList":
      return "MATCH LIST QUESTION";
    // case "matchListOptionFormat":
...
export function QuestionContent({
  ...
  switch (question.type) {
    ...
    case "caseStudy":
      return (
        <CaseStudyQuestion
          question={question as CaseStudyTestQuestion}
          currentAnswer={currentAnswer}
          onAnswer={onAnswer}
          isSubmitted={isSubmitted}
          isDisabled={isSubmitted || isViewMode}
...
export const TestEnvironment: React.FC<TestEnvironmentProps> = ({
  ...
  }, [timeRemaining, timerEnabled, timerStarted, isSubmitted, isTeacherMode, isViewMode]);
  ...
  useEffect(() => {
    if (timerEnabled && timerStarted && !isSubmitted && !isTeacherMode && !isViewMode) {
  ...
  const isSubjectTestWithSections = !isFullSyllabus && !!questionSections && questionSections.length > 0;
  // Derive the section of the current question for auto-highlighting
  const activeQuestionSection = isSubjectTestWithSections
    ? ((questions[currentQuestionIndex] as any)?.section as string | null) ?? null
    : null;
  ...
  const calculateResults = (): TestResults => {
    ...
    questions.forEach((q) => {
      ...
      if (!isAttempted(userAnswer)) {
        unattempted++;
        score += rule.unattempted;
      } else if (checkCorrectness(q, userAnswer)) {
        correct++;
        score += rule.correct;
      } else {
        wrong++;
        score += rule.incorrect;
      }
    });
    ...
    <>
      ...
      <div className="flex flex-col h-full w-full bg-slate-50/30 overflow-hidden relative">
        ...
        {isSubjectTestWithSections && (
          <div className="sticky top-16 z-20 flex items-center gap-2 px-4 bg-white border-b border-slate-200 overflow-x-auto hide-scrollbar">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider shrink-0 py-2">
              Section:
            </span>
            <div className="flex items-center gap-2 py-2 pl-1 flex-nowrap">
              {questionSections!.map((sec) => {
                const count = questions.filter((q) => (q as any).section === sec).length;
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      ...
                      const firstIdx = questions.findIndex((q) => (q as any).section === sec);
                      if (firstIdx !== -1) {
                        trackQuestionTime();
                  ...
                  >
                    {sec}
                    <span className={`text-xs font-bold ml-1 ${ isActive ? "opacity-80" : "opacity-50" }`}>
                      ({attempted}/{count})
                    </span>
                  </button>
                );
        ...
        <div className="relative flex-1 overflow-y-auto px-4 md:px-8 flex justify-center pb-20">
                ...
                <div className="flex-1">
                  {currentQuestion.type !== "mcq" && (
                    <span className="inline-block text-[10px] font-bold uppercase px-2 py-0.5 rounded-full bg-indigo-100 text-indigo-600 mb-2">
                      {getFullTypeLabel(currentQuestion.type)}
                    </span>
                  )}
