...
const caseStudyMultiCorrectSubSchema: Schema = {
  ...
  properties: {
    ...
    correctAnswerIndices: multiCorrectItemSchema.properties!.correctAnswerIndices!,
    explanation: multiCorrectItemSchema.properties!.explanation!,
    hint: multiCorrectItemSchema.properties!.hint!,
  },
  required: ["id", "question", "options", "correctAnswerIndices", "explanation"],
};
...
const caseStudyMatchListSubSchema: Schema = {
  ...
  required: ["id", "question", "listA", "listB", "correctMatches", "explanation"],
};
...
const caseStudyMatchListOptionFormatSubSchema: Schema = {
  ...
  required: ["id", "question", "listI", "listII", "options", "correctIndex", "explanation"],
};
...
const caseStudyItemSchema: Schema = {
  ...
  properties: {
    ...
    mcqSubQuestions: {
      ...
      description: "MCQ sub-questions (single-correct, 4 options). Populate when sub-questions of type MCQ are needed.",
      nullable: true,
    },
    numericalSubQuestions: {
      type: Type.ARRAY,
      items: caseStudyNumericalSubSchema,
      description: "Numerical sub-questions (student types a number). Populate when sub-questions of type numerical are needed.",
      nullable: true,
    },
    multiCorrectSubQuestions: {
      type: Type.ARRAY,
      items: caseStudyMultiCorrectSubSchema,
      description: "Multi-correct sub-questions (multiple options correct). Populate when sub-questions of type multiCorrect are needed.",
      nullable: true,
    },
    matchListSubQuestions: {
      type: Type.ARRAY,
      items: caseStudyMatchListSubSchema,
      description: "Match-list sub-questions (two columns to match). Populate when sub-questions of type matchList are needed.",
      nullable: true,
    },
    matchListOptionFormatSubQuestions: {
      type: Type.ARRAY,
      items: caseStudyMatchListOptionFormatSubSchema,
      description: "Match-list option-format sub-questions. Populate when sub-questions of type matchListOptionFormat are needed.",
      nullable: true,
    },
    singleDigitIntegerSubQuestions: {
      type: Type.ARRAY,
      items: caseStudySingleDigitIntegerSubSchema,
      description: "Single-digit integer sub-questions (answer is exactly one digit 0-9). Populate when sub-questions of type singleDigitInteger are needed.",
      nullable: true,
    },
    explanation: {
      type: Type.STRING,
...
export const buildTestResponseSchema = (sectionNames: string[]): Schema => {
  const sectionEnum = sectionNames.length > 0 ? sectionNames : undefined;

  const patchSection = (itemSchema: Schema): Schema => {
    if (!sectionEnum || !itemSchema.properties?.section) return itemSchema;
    return {
      ...itemSchema,
      properties: {
  ...
  return {
    ...testResponseSchema,
    properties: {
      ...testResponseSchema.properties,
      mcq: { ...testResponseSchema.properties!.mcq!, items: patchSection(mcqItemSchema) },
      numerical: { ...testResponseSchema.properties!.numerical!, items: patchSection(numericalItemSchema) },
      multiCorrect: { ...testResponseSchema.properties!.multiCorrect!, items: patchSection(multiCorrectItemSchema) },
      matchList: { ...testResponseSchema.properties!.matchList!, items: patchSection(matchListItemSchema) },
      matchListOptionFormat: { ...testResponseSchema.properties!.matchListOptionFormat!, items: patchSection(matchListOptionFormatItemSchema) },
      caseStudy: { ...testResponseSchema.properties!.caseStudy!, items: patchSection(caseStudyItemSchema) },
      singleDigitInteger: { ...testResponseSchema.properties!.singleDigitInteger!, items: patchSection(singleDigitIntegerItemSchema) },
    },
  };
};
