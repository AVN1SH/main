...
export default function TestSettingsPage() {
      ...
      subjects.map(async (subj) => {
        ...
        try {
          const [listRes, secRes] = await Promise.all([
            fetch(`/api/syllabus/list?class=${encodeURIComponent(gradeId)}&subject=${encodeURIComponent(subjKey)}`),
            fetch(`/api/syllabus/sections?class=${encodeURIComponent(gradeId)}&subject=${encodeURIComponent(subjKey)}`),
          ]);
          if (listRes.ok) {
  ...
  const toggleLinkedSection = (subjKey: string, docId: string) => {
    setLinkedSectionsPerSubject((prev) => {
      const current = prev[subjKey] || [];
      const updated = current.includes(docId)
        ? current.filter((id) => id !== docId)
        : [...current, docId];
      return { ...prev, [subjKey]: updated };
    });
  };
  ...
  const saveLinkedSections = async (subjKey: string) => {
    ...
    try {
      const res = await fetch("/api/syllabus/sections", {
        method: "PATCH",
                    ...
                    {currentConfig.subjects.map((subj: string) => {
                      ...
                      return (
                        <div key={subjectKey} className="border border-slate-100 rounded-2xl overflow-hidden">
                          ...
                          {/* ── Linked Syllabus Sections — only shown when this subject has sections ── */}
                          {sections.length > 0 && (
                            <div className="px-5 py-4 bg-indigo-50/40 border-t border-indigo-100 space-y-3">
                              <div className="flex items-center justify-between">
                                <div>
                                  <p className="text-xs font-black text-indigo-700 uppercase tracking-widest">
                                    Linked Syllabus Sections
                                  </p>
                                  <p className="text-[10px] text-slate-400 mt-0.5">
                                    Select other subjects from the syllabus to include as section context in generation.
                                  </p>
                                </div>
                                <button
                                  onClick={() => saveLinkedSections(subjectKey)}
                                  disabled={savingLinkedSections === subjectKey}
                                  className="flex items-center gap-1 px-3 py-1.5 text-xs font-bold bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-all disabled:opacity-50"
                                >
                                  <Save className="size-3" />
                                  {savingLinkedSections === subjectKey ? "Saving…" : "Save"}
                                </button>
                              </div>
                              {(syllabusDocsPerSubject[subjectKey] || []).length === 0 ? (
                                <p className="text-[11px] text-slate-400 italic">
                                  No other syllabus subjects found for this grade.
                                </p>
                              ) : (
                                <div className="flex flex-wrap gap-2">
                                  {(syllabusDocsPerSubject[subjectKey] || []).map((doc: any) => {
                                    const isLinked = (linkedSectionsPerSubject[subjectKey] || []).includes(doc._id);
                                    return (
                                      <button
                                        key={doc._id}
                                        onClick={() => toggleLinkedSection(subjectKey, doc._id)}
                                        className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all border ${
                                          isLinked
                                      ...
                                      >
                                        {isLinked ? "✓ " : ""}{doc.label}
                                        <span className="ml-1 opacity-50 text-[10px]">{doc.exam_type}</span>
                                      </button>
                                    );
                                  })}
                                </div>
