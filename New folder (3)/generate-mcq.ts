...
import { buildTestResponseSchema, testResponseSchema } from "@/schemas/test.schema";
...
import { getGradeConfig, type GradeConfig, type SubjectSectionConfig, type CaseStudyConfig } from "@/utils/testConfig";
import {
...
const buildCaseStudyDescription = (csConfig?: CaseStudyConfig): string => {
  ...
  const allowedTypes = csConfig.subQuestionTypes.length > 0 ? csConfig.subQuestionTypes : ["mcq"];
  const arrayNames = allowedTypes.map(t => typeToArrayName[t] || t).join(", ");
  return `Case study / passage-based questions. Generate exactly ${csConfig.count} case study passage(s), each with exactly ${csConfig.subQuestionsPerCase} sub-question(s) total. Allowed sub-question types: ${allowedTypes.join(", ")}. Place sub-questions in their matching typed array: ${arrayNames}. Each array enforces the exact schema for that question type. IMPORTANT: Each sub-question counts as a separate question in the total count.`;
};
  ...
  for (const section of SECTION_KEYS) {
    ...
    if (items && Array.isArray(items)) {
      for (const q of items) {
        ...
        const sectionLabel = (q as any).section === "null" || (q as any).section === "" || (q as any).section === null ? null : (q as any).section;
        ...
        if (section === "caseStudy") {
          ...
          for (const [arrayKey, subType] of Object.entries(SUB_QUESTION_ARRAYS)) {
...
const applyPaperConfig = (gradeConfig: GradeConfig, paper?: string): GradeConfig => {
  ...
  return {
    ...
    fullTest: {
      ...
      ...(pc.totalQuestions != null ? { totalQuestions: pc.totalQuestions } : {}),
...
const generateQuestionsForSubject = async (
  ...
  if (paperType === "Chapter Test") {
    ...
    if (selectedChapters && selectedChapters.length > 0) {
      ...
      const chapterQuery = selectedChapters.join(", ");
      ...
      try {
        ...
        if (enhancementResult?.text) enhancedQuery = enhancementResult.text.trim();
      } catch { /* fallback to raw chapter names */ }
      ...
      if (syllabus) {
        const filteredWeightage = syllabus.chapter_weightage.filter((cw: any) =>
          selectedChapters.some((sc) =>
            cw.chapter.toLowerCase().includes(sc.toLowerCase()) ||
            sc.toLowerCase().includes(cw.chapter.toLowerCase()),
          ),
        );
        context +=
          "\n\nSelected Chapters Syllabus (focus questions ONLY on these chapters): " +
          JSON.stringify(filteredWeightage.length > 0 ? filteredWeightage : syllabus.chapter_weightage);
      }
    } else {
      ...
      if (syllabus) {
        ...
        if (linkedSections && linkedSections.length > 0) {
          context +=
            "\n\nLinked Subject Sections (treat each as a distinct section of this subject; use their chapter weightage to distribute questions across these sections accordingly):\n" +
            linkedSections
              .map(
                (sec: any) =>
  ...
  const selectedChaptersStr = selectedChapters && selectedChapters.length > 0
    ? ` focusing on chapters: ${selectedChapters.join(", ")}`
    : chapter ? ` focusing on chapter "${chapter}"` : "";
...
- Always enclose Mathematical expressions strictly between single dollar signs ($...$) and you must use latex format where needed, including (question opitons, hints, explanation, etc).
...
- Provide a helpful hint for each question (formula or concept used).`;
  const result = await ai.models.generateContent({
    ...
    config: {
      ...
      responseSchema: hasSectionalConfig ? buildTestResponseSchema(enabledSections?.map((s) => s.sectionName) ?? []) : testResponseSchema,
      systemInstruction,
    },
  });
