...
import type { TestQuestion, ScoringConfig, TestResults } from "@/types/global";
import { checkCorrectness, isAttempted, QuestionContent } from "./TestEnvironment";
...
interface TestReviewProps {
  ...
  setReattemptDialog: React.Dispatch<React.SetStateAction<"confirm" | "open-test" | null>>;
...
export default function TestReview({
  ...
  // Section filter for Subject Test review
  const isSubjectTestWithSections = !isFullSyllabus && !!questionSections && questionSections.length > 0;
  const [currentSection, setCurrentSection] = useState<string>(
    isSubjectTestWithSections && questionSections ? questionSections[0] : ""
  );
  ...
  const filteredQuestions =
    isMultiSubject && currentSubject
      ...
      : isSubjectTestWithSections && currentSection
      ? questions.filter((q) => (q as any).section === currentSection)
      : questions;
  ...
  const subjectResults = useMemo(() => {
    const subjectQs = isMultiSubject && currentSubject
      ? questions.filter((q) => q.subject === currentSubject)
      : questions;
  ...
  const getQuestionScore = (q: typeof questions[0], isCorrect: boolean, attempted: boolean): { marks: number; maxMarks: number } => {
    ...
    return { 
      marks: isCorrect ? rule.correct : rule.incorrect, 
  ...
  const handleFilterChange = (filter: "all" | "correct" | "wrong" | "skipped") => {
    setFilterLoading(true);
                ...
                {isMultiSubject && (
                  <div className="flex-1 w-full sm:w-auto overflow-x-auto hide-scrollbar">
                    <div className="flex items-center gap-2 py-1">
                      <span className="text-xs font-bold text-slate-400 uppercase tracking-wider whitespace-nowrap px-2">
                        Subject:
                      </span>
                    ...
                    <div className="flex items-center gap-2 py-1">
                      ...
                      <div className="flex gap-1 flex-nowrap">
                        {questionSections!.map((sec) => {
                          ...
                          const secQs = questions.filter((q) => (q as any).section === sec);
                          const secCorrect = secQs.filter((q) => checkCorrectness(q, userAnswers[q.id])).length;
                          return (
                            <button
                              key={sec}
