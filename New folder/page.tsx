...
export default function ConfigPage() {
  ...
  const handleAddCourse = async () => {
    ...
    try {
      ...
      const courseData = {
        gradeId,
        gradeName: gradeId, // use input as display name initially
              ...
              {gradeSteps.map((grade) => (
                ...
                >
                  <div className="flex flex-col min-w-0">
                    <span className="truncate">{grade.gradeName || grade.gradeId}</span>
                    {grade.gradeName && grade.gradeName !== grade.gradeId && (
                      <span className="text-[10px] font-mono text-slate-400 mt-0.5 truncate">{grade.gradeId}</span>
                    )}
                  </div>
                  <button
                    onClick={(e) => {
