...
export const calculateQuestionScore = (
  ...
  if (checkCorrectness(q, answer)) {
    ...
    if ((q.type === "multiCorrect" || q.type === "matchListOptionFormat") && rule.partialPerCorrect !== undefined) {
      const numCorrect = q.type === "multiCorrect"
        ? (q as MultiCorrectTestQuestion).correctAnswerIndices.length
        : ((q as MatchListOptionFormatTestQuestion).correctIndices || [(q as MatchListOptionFormatTestQuestion).correctIndex].filter(i => i !== undefined)).length;
  ...
  switch (q.type) {
    ...
    case "matchListOptionFormat": {
      ...
      const correctIndices = mlofQ.correctIndices || (mlofQ.correctIndex !== undefined ? [mlofQ.correctIndex] : []);
      const selected = (answer || "").split(",").filter(Boolean);
...
export const TestEnvironment: React.FC<TestEnvironmentProps> = ({
  ...
  const calculateResults = (): TestResults => {
    ...
    questions.forEach((q) => {
      ...
      // For case study sub-questions, prefer a sub-type specific rule (e.g. "caseStudy_mcq")
      // then fall back to the generic "caseStudy" rule, then the default rule.
      const rule = isCaseStudySub
        ? (scoring[`caseStudy_${q.type}`] || scoring["caseStudy"] || defaultRule)
        : (scoring[q.type] || defaultRule);
              ...
              <div className="flex justify-between items-start mb-6 relative z-10">
                ...
                <div className="absolute -top-[35px] md:-top-8 -right-1 md:-right-3 bg-white flex items-center gap-2">
                  <div className="flex items-center gap-1.5 font-bold text-xs border px-1 md:px-2 rounded-full py-0.5 md:py-1">
                    ...
                    {(() => {
                      const isCaseStudySub = !!(currentQuestion as any)._caseStudyParentId;
                      const lookupType = isCaseStudySub ? "caseStudy" : currentQuestion.type;
                    ...
                    {(() => {
                      ...
                      if (!rule) return null;
                      if ((lookupType === "multiCorrect" || lookupType === "matchListOptionFormat") && (rule.partialPerCorrect ?? 0) > 0) {
                        return (
                      ...
                      if (lookupType === "caseStudy" && (rule.partialPerCorrect ?? 0) > 0) {
                        return (
                          <span className="text-amber-600 bg-amber-50 px-2 rounded-lg border border-amber-100">
                      ...
                      if (lookupType === "matchList" && (rule.partialPerMatch ?? 0) > 0) {
                        return (
                          <span className="text-amber-600 bg-amber-50 px-2 rounded-lg border border-amber-100">
