...
export interface GradeConfig {
  ...
  caseStudyConfig?: CaseStudyConfig;
  /**
   * Board-specific question count overrides (only for school grades).
   * Key = board name (e.g. "CBSE"), value = per-section count override.
   * Falls back to `questionCount` for any section not overridden.
   */
  boardQuestionCount?: Record<string, Record<string, { min: number; max: number; default?: number }>>;
}
...
/**
 * Applies board-specific question count overrides to a GradeConfig.
 * Called in generate-mcq after applyPaperConfig.
 * Only merges counts from boardQuestionCount[board] — all other config stays the same.
 */
export const applyBoardConfig = (gradeConfig: GradeConfig, board?: string): GradeConfig => {
  if (!board || board === "default" || !gradeConfig.boardQuestionCount) return gradeConfig;
  const boardQCs = gradeConfig.boardQuestionCount[board];
  if (!boardQCs || Object.keys(boardQCs).length === 0) return gradeConfig;

  return {
    ...gradeConfig,
    questionCount: {
      ...gradeConfig.questionCount,
      ...boardQCs,
    },
  };
};
