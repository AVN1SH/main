...
const generateQuestionsForSubject = async (
  ...
  if (paperType === "Chapter Test") {
    ...
    if (selectedChapters && selectedChapters.length > 0) {
      ...
      const chapterQuery =
        selectedChapters.join(", ") + filteredWeightage
          ? "Syllabus : " +
            filteredWeightage
              .map((item: any) =>
                item.chapter + item.description ? " : " + item.description : "",
              )
              .join(", ")
          : "";
                ...
                if (linkedDocs.length > 0) {
                  ...
                  return (
                    ...
                    linkedDocs
                      .map(
                        (d: any) =>
                          `Subject: ${d.subject.replace(/_/g, " ")} (${d.exam_type}):\n${JSON.stringify(d.chapter_weightage)}`,
                      )
                      .join("\n\n")
                  );
                }
...
- **MULTILINE QUESTION FORMATTING (CRITICAL)**: For structured question types like Assertion-Reasoning, Statement I & II, or any question with labelled items, you MUST use markdown line breaks (\\n) inside the question string to put each part on its own line. Examples:
  - Assertion-Reasoning: "**Assertion (A):** <assertion text>\\n**Reason (R):** <reason text>\\n\\nWhich of the following is correct?"
  - Statement-based: "**Statement I:** <statement>\\n**Statement II:** <statement>\\n\\nWhich of the above statements is/are correct?"
  - Match/List questions: Use \\n between each list item.
  - NEVER collapse multi-part questions into a single unbroken sentence.
...
**FINAL CRITICAL CHECK**: Before returning the JSON, YOU MUST INTERNALLY VERIFY that the total number of questions exactly matches the requested count (e.g. 18 questions). Count every single item in the mcq, numerical, multiCorrect, matchList arrays AND count every single sub-question inside the case study arrays. If the total is less than the requested amount, YOU MUST generate the missing questions before finishing. Do NOT output a partial test.`;
  ...
  const countHints: Record<
    string,
    { desc: string; minItems?: number; maxItems?: number }
  > = {};
  ...
  for (const section of gradeConfig.sections) {
    if (section === "caseStudy" && csConfig?.enabled) {
      ...
      if (d != null) {
        ...
        countHints[section] = {
          desc: `Part of a ${gradeConfig.fullTest.subjectQuestions[subjectKey]}-question subject total`,
        };
      }
    } else {
      ...
      if (c) {
        countHints[section] = {
          desc:
            c.min === c.max
              ? `Exactly ${c.min} questions`
              : `${c.min}–${c.max} questions`,
          minItems: c.min,
          maxItems: c.max,
        };
      }
...
export async function POST(request: Request) {
  try {
    ...
    // Apply board-specific question count overrides if a board is specified
    gradeConfig = applyBoardConfig(gradeConfig, board);
    ...
    if (isFullTest && gradeSubjects.length > 0) {
      ...
      const questionsPromises = subjects.map((subj) => {
        ...
        // Pass subjectSections for each subject so Full Test also gets section-tagged questions
        const subjSections =
          gradeConfig.subjectSections?.[subjectKey]?.filter((s) => s.enabled) ?? [];
        return generateQuestionsForSubject(
          ...
          subjectKey,
          language,
          subjSections.length > 0 ? subjSections : undefined,
        );
      });
    ...
    // Collect distinct section labels for Subject Test and Full Test tab rendering
